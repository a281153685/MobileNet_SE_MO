...
class BasicConv(nn.Module):
    def __init__(self, input_channel, out_channel, kernel_size, stride=1, padding=0, dilation=1, groups=1,
                 relu=True, bn=True, bias=False):
        super(BasicConv, self).__init__()
        self.output_channel = out_channel
        self.conv = nn.Conv2d(input_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=padding,
...
def logsumexp_2d(tensor):
    ...
    outputs = s + (tensor_flatten - s).exp().sum(dim=2, keepdims=True).log()
    return outputs
...
class CBAM(nn.Module):
    def __init__(self, gate_channel, reduction_ratio=16, pool_types=['avg', 'max'], no_spatial=False):
